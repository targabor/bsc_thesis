...
def basic_median_ever_frame():
    try:
        assert kernel_entry.get(), messagebox.showerror("Kernel size error",
                                                        "The kernel size is empty, please enter a value")
        assert int(kernel_entry.get()) > 1, messagebox.showerror("Kernel size error",
                                                                 "Kernel size must be greater than one!")
...
def weighted_median_ever_frame():
    try:
        ...
        assert int(kernel_entry.get()) > 1, messagebox.showerror("Kernel size error",
                                                                 "Kernel size must be greater than one!")
        assert int(kernel_entry.get()) % 2 == 1, messagebox.showerror("Kernel Error", "Kernel must be odd")
        cpp_caller.call_weighted_median_for_video_frame('', int(kernel_entry.get()), weight_type.get(),
                                                        input_text.get(), output_text.get())
        messagebox.showinfo("Done", "Weighted median filter ended.")
    except Exception as e:
        messagebox.showerror("There is an error!", str(e))
...
def dir_w_median_ever_frame():
    try:
        assert threshold_entry.get(), messagebox.showerror("Threshold error",
                                                           "The Threshold is empty, please enter a value")
        cpp_caller.call_directional_weighted_median_for_video_frame('', int(threshold_entry.get()),
                                                                    input_text.get(), output_text.get())
        messagebox.showinfo("Done", "Directional weighted median filter ended.")
    except Exception as e:
        messagebox.showerror("There is an error!", str(e))
...
def basic_median_cube():
    try:
        ...
        assert int(kernel_entry.get()) > 1, messagebox.showerror("Kernel size error",
                                                                 "Kernel size must be greater than one!")
        assert int(kernel_entry.get()) % 2 == 1, messagebox.showerror("Kernel Error", "Kernel must be odd")
        assert frames_entry.get(), messagebox.showerror("Frame error",
                                                        "The frame is empty, please enter a value")
        cpp_caller.call_simple_median_cube('', int(kernel_entry.get()),
                                           int(frames_entry.get()),
                                           input_text.get(), output_text.get())
        messagebox.showinfo("Done", "Basic median cube filter ended.")
    except Exception as e:
        messagebox.showerror("There is an error!", str(e))
...
def two_pass_cube():
    try:
        assert frames_entry.get(), messagebox.showerror("Frame error",
                                                        "The frame is empty, please enter a value")
        cpp_caller.call_two_pass_median_cube('', int(frames_entry.get()), input_text.get(), output_text.get())
        messagebox.showinfo("Done", "Two pass median cube filter ended.")
    except Exception as e:
        messagebox.showerror("There is an error!", str(e))
...
def weighted_median_cube():
    try:
        ...
        assert frames_entry.get(), messagebox.showerror("Frame error",
                                                        "The frame is empty, please enter a value")
        cpp_caller.call_weighted_median_cube('', int(kernel_entry.get()), weight_type.get(), int(frames_entry.get()),
                                             input_text.get(), output_text.get())
        messagebox.showinfo("Done", "Weighted median cube filter ended.")
    except Exception as e:
        messagebox.showerror("There is an error!", str(e))
...
def dir_w_median_cube():
    try:
        ...
        assert frames_entry.get(), messagebox.showerror("Frame error",
                                                        "The frame is empty, please enter a value")
        cpp_caller.call_dir_w_median_cube('', int(threshold_entry.get()), int(frames_entry.get()),
                                          input_text.get(), output_text.get())
        messagebox.showinfo("Done", "Directional weighted median cube filter ended.")
    except Exception as e:
        messagebox.showerror("There is an error!", str(e))
